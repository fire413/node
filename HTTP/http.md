...
**HTTP报文结构**：head+body结构，具体是：起始行+头部+空行+实体

起始行由于请求报文与相应报文的不同，有不同的区别

**请求报文的起始行为**：方法+路径+HTTP版本，例如：GET /home  HTTP/1.1，请求报文起始行有别名，别名为请求行

**响应报文的起始行为**：HTTP版本+状态+原因，例如：HTTP/1.1 200 OK，响应报文起始行有别名，别名为状态行

![image-20200921111019854](https://i.loli.net/2020/09/21/Kkh1pZnAvFSD39Y.png)

![image-20200921111129389](https://i.loli.net/2020/09/21/rHbxn2T1vFX6ARi.png)

**请求头与响应头头部字段注意事项**：

1.不区分大小写

2.字段名之间不允许有空隔，不允许有下划线

3.字段名后面必须紧接着“：”

空行用于区分头部与实体，空行后面被视为实体，空行不可或缺

请求报文的实体叫做请求体，响应报文的实体叫做响应体
...
- 从**TCP**的角度，GET 请求会把请求报文一次性发出去，而 POST 会分为两个 TCP 数据包，首先发 header 部分，如果服务器响应 100(continue)， 然后发 body 部分。(**火狐**浏览器除外，它的 POST 请求只发一个 TCP 包)
  ...
  URI全称（Uniform Resource Identifier）,也是统一资源标识符，为区分互联网上不同的资源。

  注意：它并不是我们常说的`网址`, 网址指的是`URL`, 实际上`URI`包含了`URN`和`URL`两个部分，由于 URL 过于普及，就默认将 URI 视为 URL 了。

  URI完整结构：

  ![image-20200921105950870](https://i.loli.net/2020/09/21/ReHEimID59Q8MGK.png)
...
1.灵活可扩展：语义自由，没有严格语法限制，传输形式多样，客串图片视频等。
...
Accept: 标明 需要 什么 类型的数据

响应：

Content-type: 标明 返回 的 数据 类型
...
Accept-Encoding: 代表 数据 以 什么 方式 来 传输

接收：

Content-Encoding: 标明 返回 的数据 以什么 方式 压缩 数据
...
Accept-Language: 标明 返回数据 的 语言 类型

响应：

Content-Language: 标明 返回 的数据 是什么 语言 类型
...
请求用Accept-Charset，响应用Content-Type
...
![image-20200921153217455](https://i.loli.net/2020/09/21/DfWMz54PZ329yUl.png)

## HTTP传输定长数据与不定长数据

响应头设置为：

传输定长数据，例如：Content-Length： 长度值;

传输不定长数据，：Transfer-Encoding: chunked

## 处理大文件传输

采用范围请求的解决方案，将资源一部分一部分地传输

注意：服务器收到请求之后，首先验证范围**是否合法**，如果越界了那么返回`416`错误码，否则读取相应片段，返回`206`状态码。

请求头字段的设置，格式为Range：bytes=x-y
...
由于HTTP的一发一收机制，串行执行任务，一旦队首请求太慢，会导致后面的任务请求阻塞。

并发连接：一个域名运行分配多个长连接，谷歌分发上限是6个。
...
![img](https://mmbiz.qpic.cn/mmbiz_png/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDh60iajlY6cIjXllucJ2GcPHqZL2pZUvibb2rhDibLndrVMX0AHuybLHIQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDGLwKK0SY3RTD930JffJWcBA7orodviacibt9HVDAVNaWYPibBiaX0PlvAw/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

![img](https://mmbiz.qpic.cn/mmbiz_jpg/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDJk7zdWMIOJUiaw4XJ0BWH5fY7EnU24zf0EOcgqa8uog5eWQtlEQ4Xkg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

**WebKit 渲染引擎**和**V8 引擎**都在渲染进程当中。

当`xhr.send`被调用，即 Ajax 请求准备发送的时候，其实还只是在渲染进程的处理。为了防止黑客通过脚本触碰到系统资源，浏览器将每一个渲染进程装进了沙箱，并且为了防止 CPU 芯片一直存在的**Spectre** 和 **Meltdown**漏洞，采取了`站点隔离`的手段，给每一个不同的站点(一级域名不同)分配了沙箱，互不干扰。具体见YouTube上Chromium安全团队的演讲视频。
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDtX3JdNU7VcZrgMgHf33PwicpsTbS2RvPAQ1vxel2mJ4cQVAkwA7Vicqg/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDJQfOCnrYia6h1hlZC2wgsbZQvNfJBFXId6Qry0CC6q2rRKJ4cyHbMwA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDibTyrd4dic9EkpbGlURgpwYeGHusQYEa9uLGG6Bfd3uShP0DWRYNc4kA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDsvMliafJIoicrnjNSLia9pkgsiaibF9wtBTz0f4kDOLWA9mcq70UiaA1D9FQ/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDc9ISsFghRicxVZAYX7M6oRia5FT1lufGcEJDKMmoBYkfiayoezRKljfQg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDbibRCrcibpjHib1UOGVVb1mtbNPPGLbicjopPLYCpSXQ7047Dnec7A1nAw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDibF1ibLEDSJoU0icuxOIyWWibO5IgxvqjHiaARAGiavmjH1hUxKib78TWlZ0Q/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/OFeiaAoahHJKWbCOQ9McicOaRPa5Jbr7iaDercNUjHl9M2p6lf9uUZppghRYK1R6iaRBliceF6falVSiaibVrbicCSTu2A/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
